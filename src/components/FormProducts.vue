    ...
    <form @submit.prevent="submitProduct" class="space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <!-- Dropdown menu -->
              <div v-show="showDropdown" class="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow max-h-60 overflow-auto">
                ...
                <ul>
                  <li v-for="cat in filteredCategories" :key="cat.id">
                    <button type="button"
                            class="w-full text-left px-4 py-2 hover:bg-green-50 hover:text-green-700"
                            @click="selectCategory(cat)">
                      {{ cat.name }}
                    </button>
                  </li>
                </ul>
                <div v-if="filteredCategories.length === 0" class="px-4 py-2 text-gray-400 text-sm">No results found</div>
              </div>
            </div>
          </div>
        ...
        <div class="flex flex-col items-center justify-start space-y-4">
          ...
          <div v-if="previewImage" class="w-full h-64 border rounded-lg overflow-hidden flex items-center justify-center">
      ...
      <div class="flex justify-end space-x-4 mt-6">
        <button type="button" @click="cancel" class="px-6 py-2 bg-gray-300 rounded-lg hover:bg-gray-400 transition">Cancel</button>
